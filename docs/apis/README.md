...
> 本文档列出了所有可用的 API 接口，包括请求参数和返回结果示例。
...
- [基金搜索](#基金搜索)
- [基金搜索（以名称）](#基金搜索（以名称）)
- [获取基金列表（按字母）](#获取基金列表（按字母）)
- [获取基金列表（按类型）](#获取基金列表（按类型）)
- [获取基金列表（所有-简单）](#获取基金列表所有-简单)
- [获取基金公司列表（所有）](#获取基金公司列表（所有）)
...
- [获取基金评级](#获取基金评级)
- [获取基金涨幅](#获取基金涨幅)
- [获取基金同类排名走势](#获取基金同类排名走势)
- [获取基金的基金经理](#获取基金的基金经理)
- [获取基金累计收益](#获取基金累计收益)
- [获取基金净值](#获取基金净值)
- [获取基金净值估算（实时）](#获取基金净值估算（实时）)
- [获取基金历史净值](#获取基金历史净值)

### 基金公司
- [获取基金公司基础信息（旗下基金和相关主题）](#获取基金公司基础信息旗下基金和相关主题)
- [获取基金公司基础信息（旗下基金列表）](#获取基金公司基础信息旗下基金列表)
- [获取基金公司基础信息（旗下基金分类）](#获取基金公司基础信息旗下基金分类)
- [获取基金公司基础信息（基本情况）](#获取基金公司基础信息基本情况)
- [获取基金公司基础信息（规模变动）](#获取基金公司基础信息规模变动)

### 基金经理
- [获取基金经理信息](#获取基金经理信息)
- [获取基金经理业绩走势](#获取基金经理业绩走势)
- [获取基金经理业绩排行](#获取基金经理业绩排行)
- [获取基金经理业绩评价](#获取基金经理业绩评价)
- [获取基金经理风格](#获取基金经理风格)
- [获取基金经理持仓特点](#获取基金经理持仓特点)
- [获取基金经理持仓历史管理资金](#获取基金经理持仓历史管理资金)
...
> 通过 Server-Sent Events (SSE) 实时获取股票/指数行情数据
...
该接口返回 `text/event-stream` 类型的响应，用于实时推送主要股票指数的行情数据。
...
while (true) {
  ...
  console.log('实时数据:', text);
}
```

**注意事项**

1. SSE 连接是长连接，请确保在不需要时关闭连接
2. 数据推送频率取决于服务端配置
3. 如果连接断开，建议实现重连机制
